(function( $ ) {
	...
	setTimeout(function(){
		...
		if( jQuery('.tdws_auto_completed_status').length > 0 ){
			jQuery('.tdws_auto_completed_status').select2({placeholder : jQuery('.tdws_auto_completed_status').attr('data-placeholder'), allowClear: true});	
		}
	},1500);

	// TDWS Coupon Date

	jQuery(document).on( 'focus click', '.tdws_coupon_expiry_date', function(e) {
		if( !jQuery(this).hasClass('hasDatepicker') ){
			jQuery( ".tdws_coupon_expiry_date" ).datepicker({
				'dateFormat' : 'yy-mm-dd',
				'minDate' : 0
			});
		}
	...
	jQuery(document).on('click', '.tdws_single_upload_image_button', function (e) {
		e.preventDefault();
		...
		custom_uploader.on('select', function () { 
			...
			$(button).siblings('.tdws_single_remove_image_button').show();
		...
		* Remove image event
		*/
	$('body').on('click', '.tdws_single_remove_image_button', function (e) {
		e.preventDefault();
	...
	jQuery(document).on('click', '.tdws-single-upload-medias ul li i.delete-img', function () {
		var ids = [];
		var attach_id =  jQuery(this).parents('li').attr('data-attechment-id');
		jQuery('.tdws-single-upload-medias ul li').each(function () {
			if( attach_id != jQuery(this).attr('data-attechment-id') ){
				ids.push(jQuery(this).attr('data-attechment-id'));  
			}
		});

		jQuery(this).parents('.tdws-single-upload-medias').find('input[type="hidden"]').val(ids);
